...
export const Loading: React.FC<{ message?: string; variant?: 'spinner' | 'skeleton' }> = ({
  message = 'Loading cinema equipment...',
  variant = 'spinner',
}) => {
  if (variant === 'skeleton') {
    return (
      <div className="w-full space-y-4 animate-pulse">
        <div className="h-48 bg-white/[0.04] rounded-2xl border border-white/[0.06]" />
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div key={i} className="h-72 bg-white/[0.03] rounded-2xl border border-white/[0.05]" />
          ))}
        </div>
      </div>
    );
