    ...
    <div className="rounded-3xl border border-cinema-border bg-cinema-card p-6 sm:p-8 space-y-6 shadow-cinema">
      <div className="flex items-center justify-between border-b border-cinema-border pb-4">
        <div>
          ...
          <p className="text-xs text-cinema-muted">Manage destination sets for express gear delivery</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {addresses.map((addr) => (
          <div
            key={addr.id}
            className="rounded-2xl border border-cinema-border bg-cinema-elevated p-5 space-y-3 relative group hover:border-accent/50 hover:bg-cinema-elevated/90 transition"
          >
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] font-bold bg-accent/15 text-accent border border-accent/30 px-2 py-0.5 rounded-full">
                Verified
              </span>
            </div>

            <div className="space-y-1 text-xs">
              <div className="font-bold text-cinema-text text-sm">{addr.full_name}</div>
              <div className="text-cinema-muted">{addr.line1}</div>
              <div className="text-cinema-muted">{addr.city}, {addr.state} - {addr.pincode}</div>
              <div className="text-cinema-text font-semibold flex items-center gap-1 pt-1">
